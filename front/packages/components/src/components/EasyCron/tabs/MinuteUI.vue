<template>
  <div :class="`${prefixCls}-config-list`">
    <ElRadioGroup v-model:value="type">
      <div class="item">
        <ElRadio :value="TypeEnum.every" v-bind="beforeRadioAttrs">每分</ElRadio>
      </div>
      <div class="item">
...
import { ElInputNumber, ElRadioGroup, ElCheckboxGroup, ElCheckbox, ElRadio } from 'element-plus';
import { useTabProps, useTabEmits, useTabSetup } from './useTabMixin';

export default defineComponent({
  name: 'MinuteUI',
  components: { ElInputNumber, ElRadioGroup, ElCheckboxGroup, ElCheckbox, ElRadio },
  props: useTabProps({
    defaultValue: '*',
  }),
  emits: useTabEmits(),
