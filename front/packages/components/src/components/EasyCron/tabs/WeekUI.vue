  ...
  <div :class="`${prefixCls}-config-list`">
    <ElRadioGroup v-model:value="type">
      ...
      <div class="item">
        ...
        <span> 从 </span>
        <ElSelect v-model:value="valueRange.start" :options="weekOptions" v-bind="typeRangeSelectAttrs" />
        <span> 至 </span>
        <ElSelect v-model:value="valueRange.end" :options="weekOptions" v-bind="typeRangeSelectAttrs" />
      </div>
      <div class="item">
        <ElRadio :value="TypeEnum.loop" v-bind="beforeRadioAttrs">循环</ElRadio>
        <span> 从 </span>
        <ElSelect v-model:value="valueLoop.start" :options="weekOptions" v-bind="typeLoopSelectAttrs" />
        <span> 开始，间隔 </span>
        ...
        <div class="list list-cn">
          <ElCheckboxGroup v-model:value="valueList">
            <template v-for="opt in weekOptions" :key="i">
              <ElCheckbox :value="opt.value" v-bind="typeSpecifyAttrs">{{ opt.label }}</ElCheckbox>
            </template>
          </ElCheckboxGroup>
        </div>
      </div>
...
import { ElInputNumber, ElRadio, ElRadioGroup, ElCheckbox, ElCheckboxGroup, ElSelect } from 'element-plus';
