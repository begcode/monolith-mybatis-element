...
export default defineComponent({
  ...
  props: {
    ...
    hidden: propTypes.oneOfType([Boolean, Function]).def(false),
  },
  setup(props, { attrs }) {
          ...
          <ElCollapseTransition>
            <div v-show={unref(show)} class={[`${prefixCls}-content`, 'p-20px']}>
              <ElRow gutter={0} {...unref(getBindValue)} class="outline-1px outline-[var(--el-border-color-lighter)] outline-solid">
                {props.schema
                  .filter(item => !(isFunction(item.hidden) ? item.hidden(props.data) : item.hidden))
                  .map(item => {
                    return (
...
.v-descriptions-header__title::after {
  position: absolute;
  top: 3px;
  left: -10px;
  width: 4px;
