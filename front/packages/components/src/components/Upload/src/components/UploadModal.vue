<template>
  ...
  >
    <template #centerFooter>
      <el-button @click="handleStartUpload" type="success" :disabled="!getIsSelectFile" :loading="isUploadingRef">
        {{ getUploadBtnText }}
      </el-button>
    </template>
    <div style="display: flex; align-items: center; margin-bottom: 8px">
      <ElAlert style="--el-alert-padding: 8px 0" :title="getHelpText" type="info" :closable="false" />
      <ElUpload
        action="emptyUrl"
...
export default defineComponent({
  components: { ElButton, ElUpload, ElAlert, Dialog, FileList },
  props: {
    ...uploadProps,
    prefixCls: String,
  },
  ...
  setup(props, { emit }) {
     ...
     */
    const getConfirmProps = computed((): ElButton => {
     ...
     */
    const getCancelProps = computed((): ElButton => {
      return {
        btnText: '关闭',
    ...
    function checkFileLimit() {
      if (fileListRef.value.length + previewFileList.value.length > maxNumber.value) {
        ElMessage.warning(`最多只能上传${maxNumber.value}个文件`);
        return false;
      }
      return true;
    }
    ...
    function checkFileType(file) {
      ...
      if (accept.length > 0 && !checkFileExtType(file, accept)) {
        ElMessage.error(`只能上传${accept.join(',')}格式文件`);
        return false;
      }
    ...
    function checkFileSize(size = 0) {
      if (maxSize.value && size / 1024 / 1024 >= maxSize.value) {
        ElMessage.error(`只能上传不超过${maxSize.value}MB的文件!`);
        return false;
      }
      return true;
    }
    ...
    async function uploadApiByItem(item: FileItem) {
      ...
      try {
        ...
        const result = get(data, 'urlField');
        if (result) {
