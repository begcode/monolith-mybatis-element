...
export default {
  ...
  retrieveNoPage(paginationQuery?: any): Promise<ISmsTemplate[]> {
  ...
  create(smsTemplate: ISmsTemplate): Promise<ISmsTemplate> {
    return request.post({ url: `${apiUrl}`, data: smsTemplate });
  },
  update(smsTemplate: ISmsTemplate, batchIds?: number[], batchFields?: String[]): Promise<ISmsTemplate> {
    ...
    return request.put({ url: `${apiUrl}/${smsTemplate.id}?${queryParams}`, data: smsTemplate });
  },
  ...
  syncFromSupplier(supplierId: number): Promise<any> {
    return request.get({ url: `${apiUrl}/sync-from-supplier/${supplierId}` });
  },
