...
export default {
  ...
  retrieveNoPage(paginationQuery?: any): Promise<ISmsSupplier[]> {
  ...
  create(smsSupplier: ISmsSupplier): Promise<ISmsSupplier> {
    return request.post({ url: `${apiUrl}`, data: smsSupplier });
  },
  update(smsSupplier: ISmsSupplier, batchIds?: number[], batchFields?: String[]): Promise<ISmsSupplier> {
    ...
    return request.put({ url: `${apiUrl}/${smsSupplier.id}?${queryParams}`, data: smsSupplier });
  },
