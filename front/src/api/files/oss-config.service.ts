...
export default {
  ...
  retrieveNoPage(paginationQuery?: any): Promise<IOssConfig[]> {
  ...
  create(ossConfig: IOssConfig): Promise<IOssConfig> {
    return request.post({ url: `${apiUrl}`, data: ossConfig });
  },
  update(ossConfig: IOssConfig, batchIds?: number[], batchFields?: String[]): Promise<IOssConfig> {
    ...
    return request.put({ url: `${apiUrl}/${ossConfig.id}?${queryParams}`, data: ossConfig });
  },
