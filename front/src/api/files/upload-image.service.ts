...
export default {
  ...
  retrieveNoPage(paginationQuery?: any): Promise<IUploadImage[]> {
  ...
  update(uploadImage: IUploadImage, batchIds?: number[], batchFields?: String[]): Promise<IUploadImage> {
    ...
    return request.put({ url: `${apiUrl}/${uploadImage.id}?${queryParams}`, data: uploadImage });
  },
  ...
  create(uploadImage: IUploadImage, onUploadProgress?: (progressEvent: ProgressEvent) => void, success?) {
    ...
    return request.uploadFile<IUploadImage>(
      {
        url: `${apiUrl}`,
        onUploadProgress,
      },
