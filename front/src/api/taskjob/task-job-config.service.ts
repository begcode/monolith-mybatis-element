...
export default {
  ...
  retrieveNoPage(paginationQuery?: any): Promise<ITaskJobConfig[]> {
  ...
  create(taskJobConfig: ITaskJobConfig): Promise<ITaskJobConfig> {
    return request.post({ url: `${apiUrl}`, data: taskJobConfig });
  },
  update(taskJobConfig: ITaskJobConfig, batchIds?: number[], batchFields?: String[]): Promise<ITaskJobConfig> {
    ...
    return request.put({ url: `${apiUrl}/${taskJobConfig.id}?${queryParams}`, data: taskJobConfig });
  },
