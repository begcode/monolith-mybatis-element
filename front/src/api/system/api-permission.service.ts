...
export default {
  ...
  retrieveNoPage(paginationQuery?: any): Promise<IApiPermission[]> {
  ...
  create(apiPermission: IApiPermission): Promise<IApiPermission> {
    return request.post({ url: `${apiUrl}`, data: apiPermission });
  },
  update(apiPermission: IApiPermission, batchIds?: number[], batchFields?: String[]): Promise<IApiPermission> {
    ...
    return request.put({ url: `${apiUrl}/${apiPermission.id}?${queryParams}`, data: apiPermission });
  },
  ...
  generate(): Promise<any> {
    return request.get({url: `${apiUrl}/generate`});
  },
