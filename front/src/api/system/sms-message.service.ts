...
export default {
  ...
  retrieveNoPage(paginationQuery?: any): Promise<ISmsMessage[]> {
  ...
  create(smsMessage: ISmsMessage): Promise<ISmsMessage> {
    return request.post({ url: `${apiUrl}`, data: smsMessage });
  },
  update(smsMessage: ISmsMessage, batchIds?: number[], batchFields?: String[]): Promise<ISmsMessage> {
    ...
    return request.put({ url: `${apiUrl}/${smsMessage.id}?${queryParams}`, data: smsMessage });
  },
