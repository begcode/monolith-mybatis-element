...
export default {
  ...
  retrieveNoPage(paginationQuery?: any): Promise<IAuthority[]> {
  ...
  create(authority: IAuthority): Promise<IAuthority> {
    return request.post({ url: `${apiUrl}`, data: authority });
  },
  update(authority: IAuthority, batchIds?: number[], batchFields?: String[]): Promise<IAuthority> {
    ...
    return request.put({ url: `${apiUrl}/${authority.id}?${queryParams}`, data: authority });
  },
