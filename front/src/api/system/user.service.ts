...
export default {
  ...
  find(id: number): Promise<IUser> {
    return request.get({ url: `${apiUrl}/id/${id}` });
  },
  ...
  retrieveNoPage(paginationQuery?: any): Promise<IUser[]> {
  ...
  create(user: IUser): Promise<IUser> {
    return request.post({ url: `${apiUrl}`, data: user });
  },
  update(user: IUser, batchIds?: number[], batchFields?: String[]): Promise<IUser> {
    ...
    return request.put({ url: `${apiUrl}/${user.id}?${queryParams}`, data: user });
  },
