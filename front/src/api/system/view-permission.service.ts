...
export default {
  ...
  retrieveNoPage(paginationQuery?: any): Promise<IViewPermission[]> {
  ...
  create(viewPermission: IViewPermission): Promise<IViewPermission> {
    return request.post({ url: `${apiUrl}`, data: viewPermission });
  },
  update(viewPermission: IViewPermission, batchIds?: number[], batchFields?: String[]): Promise<IViewPermission> {
    ...
    return request.put({ url: `${apiUrl}/${viewPermission.id}?${queryParams}`, data: viewPermission });
  },
  ...
  treeByLogin(): Promise<any> {
    return request.get({ url: `${apiUrl}/current-user` });
  },
