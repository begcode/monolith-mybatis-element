...
export default {
  ...
  retrieveNoPage(paginationQuery?: any): Promise<IAnnouncementRecord[]> {
  ...
  create(announcementRecord: IAnnouncementRecord): Promise<IAnnouncementRecord> {
    return request.post({ url: `${apiUrl}`, data: announcementRecord });
  },
  update(announcementRecord: IAnnouncementRecord, batchIds?: number[], batchFields?: String[]): Promise<IAnnouncementRecord> {
    ...
    return request.put({ url: `${apiUrl}/${announcementRecord.id}?${queryParams}`, data: announcementRecord });
  },
