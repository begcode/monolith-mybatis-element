...
export default {
  ...
  retrieveNoPage(paginationQuery?: any): Promise<IDictionary[]> {
  ...
  update(dictionary: IDictionary, batchIds?: number[], batchFields?: String[]): Promise<IDictionary> {
    ...
    return request.put({ url: `${apiUrl}/${dictionary.id}?${queryParams}`, data: dictionary });
  },
