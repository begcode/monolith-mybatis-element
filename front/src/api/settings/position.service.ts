...
export default {
  ...
  retrieveNoPage(paginationQuery?: any): Promise<IPosition[]> {
  ...
  create(position: IPosition): Promise<IPosition> {
    return request.post({ url: `${apiUrl}`, data: position });
  },
  update(position: IPosition, batchIds?: number[], batchFields?: String[]): Promise<IPosition> {
    ...
    return request.put({ url: `${apiUrl}/${position.id}?${queryParams}`, data: position });
  },
