...
export default {
  ...
  retrieveNoPage(paginationQuery?: any): Promise<IDepartment[]> {
  ...
  create(department: IDepartment): Promise<IDepartment> {
    return request.post({ url: `${apiUrl}`, data: department });
  },
  update(department: IDepartment, batchIds?: number[], batchFields?: String[]): Promise<IDepartment> {
    ...
    return request.put({ url: `${apiUrl}/${department.id}?${queryParams}`, data: department });
  },
