...
export default {
  ...
  retrieveNoPage(paginationQuery?: any): Promise<IBusinessType[]> {
  ...
  create(businessType: IBusinessType): Promise<IBusinessType> {
    return request.post({ url: `${apiUrl}`, data: businessType });
  },
  update(businessType: IBusinessType, batchIds?: number[], batchFields?: String[]): Promise<IBusinessType> {
    ...
    return request.put({ url: `${apiUrl}/${businessType.id}?${queryParams}`, data: businessType });
  },
