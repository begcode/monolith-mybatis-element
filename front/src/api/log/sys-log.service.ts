...
export default {
  ...
  retrieveNoPage(paginationQuery?: any): Promise<ISysLog[]> {
  ...
  create(sysLog: ISysLog): Promise<ISysLog> {
    return request.post({ url: `${apiUrl}`, data: sysLog });
  },
  update(sysLog: ISysLog, batchIds?: number[], batchFields?: String[]): Promise<ISysLog> {
    ...
    return request.put({ url: `${apiUrl}/${sysLog.id}?${queryParams}`, data: sysLog });
  },
