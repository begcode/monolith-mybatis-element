...
export default {
  ...
  retrieveNoPage(paginationQuery?: any): Promise<IUReportFile[]> {
  ...
  create(uReportFile: IUReportFile): Promise<IUReportFile> {
    return request.post({ url: `${apiUrl}`, data: uReportFile });
  },
  update(uReportFile: IUReportFile, batchIds?: number[], batchFields?: String[]): Promise<IUReportFile> {
    ...
    return request.put({ url: `${apiUrl}/${uReportFile.id}?${queryParams}`, data: uReportFile });
  },
