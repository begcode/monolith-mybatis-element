<template>
  <div :class="prefixCls" :style="getWrapStyle">
    <iframe v-loading="loading" :src="frameSrc" :class="`${prefixCls}__main`" ref="frameRef" @load="hideLoading"></iframe>
  </div>
</template>
<script lang="ts" setup>
import type { CSSProperties } from 'vue';
import { ref, unref, computed, onMounted, onUnmounted } from 'vue';
import { propTypes, useDesign } from '@begcode/components';
...
const headerHeightRef = ref(150);

const { getPrefixCls, variables } = useDesign();

const prefixCls = getPrefixCls('iframe-page');
