<template>
  ...
  <ElCard :bordered="false" class="bc-list-result-card" :bodyStyle="{ 'padding-top': '1px' }">
    <template #header>
      <div class="card-header">
        <ElButton @click="formSearch" size="small" text class="pl-0!"><Icon icon="ep:memo" />系统日志列表</ElButton>
        <ElSpace>
      ...
      if (operation) {
        if (operation.click) {
          ...
          switch (containerType) {
            case 'modal':
              modalConfig.componentName = shallowRef(SysLogEdit);
              ...
              break;
            case 'drawer':
              drawerConfig.componentName = shallowRef(SysLogEdit);
      ...
      if (operation) {
        if (operation.click) {
          ...
          switch (operation.containerType) {
            case 'modal':
              modalConfig.componentName = shallowRef(SysLogDetail);
              modalConfig.entityId = row.id;
              modalConfig.modelValue = true;
              break;
            case 'drawer':
              drawerConfig.componentName = shallowRef(SysLogDetail);
              drawerConfig.entityId = row.id;
              drawerConfig.modelValue = true;
              break;
            case 'route':
