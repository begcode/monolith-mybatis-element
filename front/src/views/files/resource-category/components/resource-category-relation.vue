...
const gridEvents = reactive<VxeGridListeners>({
  ...
  toolbarToolClick({ code }) {
    switch (code) {
      case 'new': {
        const newConfig: any = {};
        newConfig.componentName = shallowRef(ResourceCategoryEdit);
        newConfig.entityId = '';
      ...
      if (operation) {
        if (operation.click) {
          ...
          switch (containerType) {
            case 'drawer':
              drawerConfig.componentName = shallowRef(ResourceCategoryEdit);
            ...
            case 'modal':
            default:
              modalConfig.componentName = shallowRef(ResourceCategoryEdit);
