<template>
  ...
  <ElCard :bordered="false" class="bc-list-result-card" :bodyStyle="{ 'padding-top': '1px' }">
    <template #header>
      <div class="card-header">
        <ElButton @click="formSearch" size="small" text class="pl-0!"><Icon icon="ep:memo" />上传文件列表</ElButton>
        <ElSpace>
  ...
  toolButtons: [
    {
      ...
      icon: 'ep:upload',
      click: () => {
        modalConfig.componentName = shallowRef(UploadFileEdit);
        modalConfig.entityId = '';
        modalConfig.containerType = 'modal';
        modalConfig.title = '新增上传文件';
        ...
        if (operation.click) {
          ...
          switch (operation.containerType) {
            case 'modal':
              ...
              break;
            case 'drawer':
              drawerConfig.componentName = shallowRef(UploadFileEdit);
    ...
    case 'detail':
      if (operation) {
        if (operation.click) {
          ...
          switch (operation.containerType) {
            case 'modal':
              modalConfig.componentName = shallowRef(UploadFileDetail);
              modalConfig.entityId = row.id;
              modalConfig.modelValue = true;
              break;
            case 'drawer':
              drawerConfig.componentName = shallowRef(UploadFileDetail);
              drawerConfig.entityId = row.id;
              drawerConfig.modelValue = true;
              break;
            case 'route':
