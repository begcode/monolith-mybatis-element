<template>
  ...
  <ElCard :bordered="false" class="bc-list-result-card" :bodyStyle="{ 'padding-top': '1px' }">
    <template #header>
      <div class="card-header">
        <ElButton @click="formSearch" size="small" text class="pl-0!"><Icon icon="ep:memo" />消息模板列表</ElButton>
        <ElSpace>
...
const apis = {
  ...
  deleteById: apiService.files.smsTemplateService.delete,
  deleteByIds: apiService.files.smsTemplateService.deleteByIds,
  update: apiService.files.smsTemplateService.update,
};
const pageConfig = {
  title: '消息模板列表',
    ...
    switch (code) {
      case 'new':
        if (props.editIn === 'modal') {
          modalConfig.componentName = shallowRef(SmsTemplateEdit);
          modalConfig.entityId = null;
          modalConfig.modelValue = true;
          setModalProps({ visible: true });
        } else if (props.editIn === 'drawer') {
          drawerConfig.componentName = shallowRef(SmsTemplateEdit);
      ...
      if (operation) {
        if (operation.click) {
          ...
          switch (operation.containerType) {
            case 'modal':
              modalConfig.componentName = shallowRef(SmsTemplateDetail);
              modalConfig.entityId = row.id;
              modalConfig.modelValue = true;
              break;
            case 'drawer':
              drawerConfig.componentName = shallowRef(SmsTemplateDetail);
              drawerConfig.entityId = row.id;
              drawerConfig.modelValue = true;
              break;
            case 'route':
