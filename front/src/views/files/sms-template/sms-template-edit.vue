...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const relationshipApis: any = {
      supplier: async params => (await apiService.files.smsSupplierService.retrieve(params)).records,
    };
    ...
    if (smsTemplateId.value) {
      const data = await apiService.files.smsTemplateService.find(Number(smsTemplateId.value));
      if (data) {
    ...
    const saveOrUpdateApi = smsTemplateId.value ? apiService.files.smsTemplateService.update : apiService.files.smsTemplateService.create;
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
    ...
    const formGroup = reactive([
      {
        title: props.containerType === 'router' ? '消息模板' : null,
        operation: [],
        component: {
          name: 'el-form',
          props: {
