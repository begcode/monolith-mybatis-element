<template>
  ...
  <ElCard :bordered="false" class="bc-list-result-card" :bodyStyle="{ 'padding-top': '1px' }">
    <template #header>
      <div class="card-header">
        <ElButton @click="formSearch" size="small" text class="pl-0!"><Icon icon="ep:memo" />短信服务商配置列表</ElButton>
        <ElSpace>
    ...
    switch (code) {
      case 'new':
        if (props.editIn === 'modal') {
          modalConfig.componentName = shallowRef(SmsSupplierEdit);
          modalConfig.entityId = null;
          modalConfig.modelValue = true;
          setModalProps({ visible: true });
        } else if (props.editIn === 'drawer') {
          drawerConfig.componentName = shallowRef(SmsSupplierEdit);
...
const rowClickHandler = (name, operation, row) => {
  ...
  switch (name) {
      ...
      if (operation) {
        if (operation.click) {
          ...
          switch (operation.containerType) {
            case 'modal':
              modalConfig.componentName = shallowRef(SmsSupplierDetail);
              modalConfig.entityId = row.id;
              modalConfig.modelValue = true;
              break;
            case 'drawer':
              drawerConfig.componentName = shallowRef(SmsSupplierDetail);
              drawerConfig.entityId = row.id;
              drawerConfig.modelValue = true;
              break;
            case 'route':
    ...
    case 'delete':
      ...
      break;
    case 'smsTemplateSync':
      if (operation) {
        if (operation.click) {
          operation.click(row);
        } else {
          apiService.files.smsTemplateService.syncFromSupplier(row.id).then(() => {
            formSearch();
            ElMessage.success({
              message: '同步完成',
              duration: 3000,
