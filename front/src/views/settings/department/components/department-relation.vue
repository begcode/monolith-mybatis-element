...
const gridEvents = reactive<VxeGridListeners>({
  ...
  toolbarToolClick({ code }) {
    switch (code) {
      case 'new': {
        const newConfig: any = {};
        newConfig.componentName = shallowRef(DepartmentEdit);
        newConfig.entityId = '';
      ...
      if (operation) {
        if (operation.click) {
          ...
          switch (containerType) {
            case 'drawer':
              drawerConfig.componentName = shallowRef(DepartmentEdit);
            ...
            case 'modal':
            default:
              modalConfig.componentName = shallowRef(DepartmentEdit);
