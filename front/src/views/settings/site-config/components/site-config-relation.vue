...
const apis = {
  ...
  updateRelations: apiService.settings.siteConfigService.updateRelations,
};
const pageConfig = {
  title: '网站配置列表',
...
const gridEvents = reactive<VxeGridListeners>({
  ...
  toolbarToolClick({ code }) {
    switch (code) {
      case 'new': {
        const newConfig: any = {};
        newConfig.componentName = shallowRef(SiteConfigEdit);
        newConfig.entityId = '';
      ...
      case 'add': {
        ...
        result.componentName = shallowRef(SiteConfigList);
        result.entityId = '';
        if (props.editIn === 'drawer') {
      ...
      if (operation) {
        if (operation.click) {
          ...
          switch (containerType) {
            case 'drawer':
              drawerConfig.componentName = shallowRef(SiteConfigEdit);
            ...
            case 'modal':
            default:
              modalConfig.componentName = shallowRef(SiteConfigEdit);
      ...
      if (operation) {
        if (operation.click) {
          ...
          switch (operation.containerType) {
            case 'drawer':
              drawerConfig.componentName = shallowRef(SiteConfigDetail);
              drawerConfig.entityId = row.id;
            ...
            case 'modal':
            default:
              modalConfig.componentName = shallowRef(SiteConfigDetail);
              modalConfig.entityId = row.id;
