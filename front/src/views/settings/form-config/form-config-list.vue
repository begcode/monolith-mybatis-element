<template>
  ...
  <ElCard :bordered="false" class="bc-list-result-card" :bodyStyle="{ 'padding-top': '1px' }">
    <template #header>
      <div class="card-header">
        <ElButton @click="formSearch" size="small" text class="pl-0!"><Icon icon="ep:memo" />表单配置列表</ElButton>
        <ElSpace>
...
import FormConfigEdit from './form-config-edit.vue';
import FormConfigDetail from './form-config-detail.vue';
...
const apis = {
  formConfigService: apiService.settings.formConfigService,
  find: apiService.settings.formConfigService.retrieve,
  deleteById: apiService.settings.formConfigService.delete,
  deleteByIds: apiService.settings.formConfigService.deleteByIds,
  update: apiService.settings.formConfigService.update,
};
const pageConfig = {
  title: '表单配置列表',
  baseRouteName: 'systemFormConfig',
};
      ...
      if (operation) {
        if (operation.click) {
          ...
          switch (operation.containerType) {
            case 'modal':
              modalConfig.componentName = shallowRef(FormConfigDetail);
              modalConfig.entityId = row.id;
              modalConfig.modelValue = true;
              break;
            case 'drawer':
              drawerConfig.componentName = shallowRef(FormConfigDetail);
              drawerConfig.entityId = row.id;
              drawerConfig.modelValue = true;
              break;
            case 'route':
