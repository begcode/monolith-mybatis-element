...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const relationshipApis: any = {
      businessType: async params => (await apiService.settings.businessTypeService.retrieve(params)).records,
    };
    ...
    let formConfigId = ref('');
    if (props.containerType === 'router') {
      formConfigId.value = route.params?.entityId as string;
    } else {
      formConfigId.value = props.entityId as string;
    }
    const formConfig = reactive<IFormConfig>(new FormConfig());
    if (formConfigId.value) {
      const data = await apiService.settings.formConfigService.find(Number(formConfigId.value));
      if (data) {
    ...
    const submitButtonTitlePrefix = formConfigId.value ? '更新' : '保存';
    const saveOrUpdateApi = formConfigId.value
      ? apiService.settings.formConfigService.update
      : apiService.settings.formConfigService.create;
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            Object.assign(formConfig, result);
            saveOrUpdateApi(formConfig)
              .then(res => {
                Object.assign(formConfig, res);
                ElMessage({
                  type: 'success',
    ...
    const formGroup = reactive([
      {
        title: props.containerType === 'router' ? '表单配置' : null,
        operation: [],
        component: {
          name: 'el-form',
          props: {
