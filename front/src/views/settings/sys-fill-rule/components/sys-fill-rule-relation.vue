  ...
  searchForm: (): any[] => {
    ...
    return [
      ...
      },
      {
        ...
        operator: '',
        span: 8,
        componentProps: { api: null, style: 'width: 100%', valueField: 'id', labelField: 'datePattern' },
      },
    ];
  },
...
const gridEvents = reactive<VxeGridListeners>({
  ...
  toolbarToolClick({ code }) {
    switch (code) {
      case 'new': {
        const newConfig: any = {};
        newConfig.componentName = shallowRef(SysFillRuleEdit);
        newConfig.entityId = '';
      ...
      case 'add': {
        ...
        result.componentName = shallowRef(SysFillRuleList);
        result.entityId = '';
        if (props.editIn === 'drawer') {
