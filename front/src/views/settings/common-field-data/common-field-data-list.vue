<template>
  ...
  <ElCard :bordered="false" class="bc-list-result-card" :bodyStyle="{ 'padding-top': '1px' }">
    <template #header>
      <div class="card-header">
        <ElButton @click="formSearch" size="small" text class="pl-0!"><Icon icon="ep:memo" />通用字段数据列表</ElButton>
        <ElSpace>
    ...
    switch (code) {
      case 'new':
        if (props.editIn === 'modal') {
          modalConfig.componentName = shallowRef(CommonFieldDataEdit);
          modalConfig.entityId = null;
          modalConfig.modelValue = true;
          setModalProps({ visible: true });
        } else if (props.editIn === 'drawer') {
          drawerConfig.componentName = shallowRef(CommonFieldDataEdit);
      ...
      if (operation) {
        if (operation.click) {
          ...
          switch (operation.containerType) {
            case 'modal':
              modalConfig.componentName = shallowRef(CommonFieldDataDetail);
              modalConfig.entityId = row.id;
              modalConfig.modelValue = true;
              break;
            case 'drawer':
              drawerConfig.componentName = shallowRef(CommonFieldDataDetail);
              drawerConfig.entityId = row.id;
              drawerConfig.modelValue = true;
              break;
            case 'route':
