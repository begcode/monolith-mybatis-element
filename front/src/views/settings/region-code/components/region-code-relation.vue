...
const gridEvents = reactive<VxeGridListeners>({
  ...
  toolbarToolClick({ code }) {
    switch (code) {
      case 'new': {
        const newConfig: any = {};
        newConfig.componentName = shallowRef(RegionCodeEdit);
        newConfig.entityId = '';
      ...
      if (operation) {
        if (operation.click) {
          ...
          switch (containerType) {
            case 'drawer':
              drawerConfig.componentName = shallowRef(RegionCodeEdit);
            ...
            case 'modal':
            default:
              modalConfig.componentName = shallowRef(RegionCodeEdit);
