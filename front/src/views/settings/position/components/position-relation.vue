...
const apis = {
  ...
  updateRelations: apiService.settings.positionService.updateRelations,
};
const pageConfig = {
  title: '岗位列表',
...
const gridEvents = reactive<VxeGridListeners>({
  ...
  toolbarToolClick({ code }) {
    switch (code) {
      case 'new': {
        const newConfig: any = {};
        newConfig.componentName = shallowRef(PositionEdit);
        newConfig.entityId = '';
      ...
      case 'add': {
        ...
        result.componentName = shallowRef(PositionList);
        result.entityId = '';
        if (props.editIn === 'drawer') {
      ...
      if (operation) {
        if (operation.click) {
          ...
          switch (containerType) {
            case 'drawer':
              drawerConfig.componentName = shallowRef(PositionEdit);
            ...
            case 'modal':
            default:
              modalConfig.componentName = shallowRef(PositionEdit);
      ...
      if (operation) {
        if (operation.click) {
          ...
          switch (operation.containerType) {
            case 'drawer':
              drawerConfig.componentName = shallowRef(PositionDetail);
              drawerConfig.entityId = row.id;
            ...
            case 'modal':
            default:
              modalConfig.componentName = shallowRef(PositionDetail);
              modalConfig.entityId = row.id;
