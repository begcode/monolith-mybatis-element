...
const relationships = {
  'ViewPermission.authorities': 'viewPermissions',
  'ApiPermission.authorities': 'apiPermissions',
};
      ...
      if (operation) {
        if (operation.click) {
          ...
          switch (operation.containerType) {
            case 'drawer':
              drawerConfig.componentName = shallowRef(AuthorityDetail);
              drawerConfig.entityId = row.id;
            ...
            case 'modal':
            default:
              modalConfig.componentName = shallowRef(AuthorityDetail);
              modalConfig.entityId = row.id;
