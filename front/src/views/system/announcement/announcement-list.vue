<template>
  ...
  <ElCard :bordered="false" class="bc-list-result-card" :bodyStyle="{ 'padding-top': '1px' }">
    <template #header>
      <div class="card-header">
        <ElButton @click="formSearch" size="small" text class="pl-0!"><Icon icon="ep:memo" />系统通告列表</ElButton>
        <ElSpace>
...
const rowOperations = [
  ...
  },
  {
    ...
    containerType: 'drawer',
    click: row => {
      ElMessageBox.confirm(`是否确认发布ID为${row.id}的消息？`, `操作提示`, { type: 'warning' }).then(({ action }) => {
        if (action === 'confirm') {
    ...
    switch (code) {
      case 'new':
        if (props.editIn === 'modal') {
          modalConfig.componentName = shallowRef(AnnouncementEdit);
          modalConfig.entityId = null;
          modalConfig.modelValue = true;
          setModalProps({ visible: true });
        } else if (props.editIn === 'drawer') {
          drawerConfig.componentName = shallowRef(AnnouncementEdit);
      ...
      if (operation) {
        if (operation.click) {
          ...
          switch (operation.containerType) {
            case 'modal':
              modalConfig.componentName = shallowRef(AnnouncementDetail);
              modalConfig.entityId = row.id;
              modalConfig.modelValue = true;
              break;
            case 'drawer':
              drawerConfig.componentName = shallowRef(AnnouncementDetail);
              drawerConfig.entityId = row.id;
              drawerConfig.modelValue = true;
              break;
            case 'route':
