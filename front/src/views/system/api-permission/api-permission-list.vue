<template>
  ...
  <ElCard :bordered="false" class="bc-list-result-card" :bodyStyle="{ 'padding-top': '1px' }">
    <template #header>
      <div class="card-header">
        <ElButton @click="formSearch" size="small" text class="pl-0!"><Icon icon="ep:memo" />API权限列表</ElButton>
        <ElSpace>
...
const gridOptions = reactive<VxeGridProps>({
  ...
  toolbarConfig: {
    ...
    tools: [
      ...
      { code: 'generate', name: '更新', circle: false, icon: 'vxe-icon-refresh' },
    ],
  },
  columns,
});
    ...
    switch (code) {
      case 'new':
        if (props.editIn === 'modal') {
          modalConfig.componentName = shallowRef(ApiPermissionEdit);
          modalConfig.entityId = null;
          modalConfig.modelValue = true;
          setModalProps({ visible: true });
        } else if (props.editIn === 'drawer') {
          drawerConfig.componentName = shallowRef(ApiPermissionEdit);
        ...
        break;
      case 'generate': {
        apis.apiPermissionService.generate().then(res => {
