...
import ViewPermissionList from '../view-permission-list.vue';
...
const gridEvents = reactive<VxeGridListeners>({
  ...
  toolbarToolClick({ code }) {
    switch (code) {
      case 'new': {
        const newConfig: any = {};
        newConfig.componentName = shallowRef(ViewPermissionEdit);
        newConfig.entityId = '';
      ...
      case 'add': {
        ...
        result.componentName = shallowRef(ViewPermissionList);
        result.entityId = '';
        if (props.editIn === 'drawer') {
      ...
      if (operation) {
        if (operation.click) {
          ...
          switch (operation.containerType) {
            case 'drawer':
              drawerConfig.componentName = shallowRef(ViewPermissionDetail);
              drawerConfig.entityId = row.id;
            ...
            case 'modal':
            default:
              modalConfig.componentName = shallowRef(ViewPermissionDetail);
              modalConfig.entityId = row.id;
      ...
      if (operation) {
        if (operation.click) {
          ...
          switch (operation.containerType) {
            ...
            case 'route':
              if (pageConfig.baseRouteName) {
                push({ name: `${pageConfig.baseRouteName}New`, query: { parentId: row.id, parent: row } });
              } else {
                console.log('未定义方法');
              }
              break;
